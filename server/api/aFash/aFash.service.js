...
operations.getAll = function () {
	return AFash.find().populate('IDCategory IDSortSubCategory IDMUnit IDOperation materials.IDSort').lean().exec();
}

operations.getDetail = function (aFashId) {
	return AFash.findById(aFashId).populate('IDCategory IDSortSubCategory IDMUnit IDOperation materials.IDSort').lean().exec()
}
